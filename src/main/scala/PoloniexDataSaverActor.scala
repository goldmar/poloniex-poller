...
object PoloniexDataSaverActor {
  def dataToTick(timestamp: Long, currencyPair: String, bidAskMidpoint: BigDecimal,
    ...
    Tick(
      ...
      askPriceAvg1 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 1).map(_ / bidAskMidpoint - 1),
      askPriceAvg5 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 5).map(_ / bidAskMidpoint - 1),
      askPriceAvg10 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 10).map(_ / bidAskMidpoint - 1),
      askPriceAvg25 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 25).map(_ / bidAskMidpoint - 1),
      askPriceAvg50 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 50).map(_ / bidAskMidpoint - 1),
      askPriceAvg100 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 100).map(_ / bidAskMidpoint - 1),
      askPriceAvg500 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 500).map(_ / bidAskMidpoint - 1),
      askPriceAvg1000 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 1000).map(_ / bidAskMidpoint - 1),
      askPriceAvg2500 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 2500).map(_ / bidAskMidpoint - 1),
      askPriceAvg5000 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 5000).map(_ / bidAskMidpoint - 1),
      askPriceAvg10000 = aggregateItems(asks.map(i => i.price -> i.amount * i.price), 10000).map(_ / bidAskMidpoint - 1),
